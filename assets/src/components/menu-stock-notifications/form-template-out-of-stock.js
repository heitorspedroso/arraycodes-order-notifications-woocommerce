...
const PageFormTemplateOutOfStockCustomer = () => {
	...
	const handleSelectVariableFromParent = ( variable ) => {
		handleChange(
			fields.whatsapp_template_out_of_stock_customer_body + variable,
			'whatsapp_template_out_of_stock_customer_body'
		);
		closeVariableModal();
	};
	...
	const { getFields, isLoading, isLoadingStatus } = useSelect(
		( select ) => ( {
			...
			isLoadingStatus: select(
				'shop-arraycodes-order-notifications-woocommerce'
			).hasFinishedResolution( 'getOutOfStock' ),
		} ),
		[]
	);
	const [ fields, setFields ] = useState( {
		whatsapp_template_out_of_stock_customer_id: '',
		whatsapp_template_out_of_stock_customer_status: '',
		whatsapp_template_out_of_stock_customer_name: '',
		whatsapp_template_out_of_stock_customer_language: '',
		whatsapp_template_out_of_stock_customer_header: '',
		whatsapp_template_out_of_stock_customer_footer: '',
		whatsapp_template_out_of_stock_customer_body: '',
	} );
	...
	useSelect( ( select ) =>
		select( 'shop-arraycodes-order-notifications-woocommerce' ).getOutOfStock()
	);

	const handleSetStorageAndReloadPage = () => {
		window.localStorage.setItem( 'activeComponent', 'item-12' );
	...
	useEffect( () => {
		if ( getFields ) {
			...
			if ( fetchedFields.whatsapp_template_out_of_stock_customer_id ) {
				setIsHaveId( true );
			}
		}
	}, [ getFields ] );
	...
	const handleSave = async () => {
		try {
			...
			await dispatch( 'shop-arraycodes-order-notifications-woocommerce' ).saveSettings(
				{
					whatsapp_template_out_of_stock_customer_name:
					fields.whatsapp_template_out_of_stock_customer_name,
					whatsapp_template_out_of_stock_customer_language:
					fields.whatsapp_template_out_of_stock_customer_language,
					whatsapp_template_out_of_stock_customer_header:
					fields.whatsapp_template_out_of_stock_customer_header,
					whatsapp_template_out_of_stock_customer_footer:
					fields.whatsapp_template_out_of_stock_customer_footer,
					whatsapp_template_out_of_stock_customer_body:
					fields.whatsapp_template_out_of_stock_customer_body,
				},
				currentState
			);
	...
	const handleSentNewOutOfStockCustomer = async () => {
		const dataToSend = {
			fields: getFields,
		};
		...
		try {
			...
			const response = await fetch(
				'/wp-json/arraycodes-order-notifications-woocommerce/v1/new-out-of-stock-customer',
				fetchOptions
			);
	...
	const handleSentUpdateOutOfStockCustomer = async () => {
		const dataToSend = {
			fields: getFields,
		};
		const arraycodesOnVars = window.arraycodesOnVars;
		...
		try {
			...
			const response = await fetch(
				'/wp-json/arraycodes-order-notifications-woocommerce/v1/update-out-of-stock-customer',
				fetchOptions
			);
	...
	const handleSentDeleteOutOfStockCustomer = async () => {
		const dataToSend = {
			fields: getFields,
		};
		const arraycodesOnVars = window.arraycodesOnVars;
		...
		try {
			...
			const response = await fetch(
				'/wp-json/arraycodes-order-notifications-woocommerce/v1/delete-out-of-stock-customer',
				fetchOptions
			);
	...
	return (
		<div className="my-gutenberg-form">
			<SectionHeader
				title={__(
					'Message Template Out of Stock Customer',
			...
			<div className="form-legend">
				<Tip>
					{__(
						'Create the template message for out of stock messages to the customer.',
						'arraycodes-order-notifications-woocommerce'
					)}{' '}
...
export default PageFormTemplateOutOfStockCustomer;
