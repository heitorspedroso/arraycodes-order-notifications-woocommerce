...
const PageFormTemplateBackInStockCustomer = () => {
	...
	const handleSelectVariableFromParent = ( variable ) => {
		handleChange(
			fields.whatsapp_template_back_in_stock_customer_body + variable,
			'whatsapp_template_back_in_stock_customer_body'
		);
		closeVariableModal();
	};
	...
	const { getFields, isLoading, isLoadingStatus } = useSelect(
		( select ) => ( {
			...
			isLoadingStatus: select(
				'shop-arraycodes-order-notifications-woocommerce'
			).hasFinishedResolution( 'getBackInStock' ),
		} ),
		[]
	);
	const [ fields, setFields ] = useState( {
		whatsapp_template_back_in_stock_customer_id: '',
		whatsapp_template_back_in_stock_customer_status: '',
		whatsapp_template_back_in_stock_customer_name: '',
		whatsapp_template_back_in_stock_customer_language: '',
		whatsapp_template_back_in_stock_customer_header: '',
		whatsapp_template_back_in_stock_customer_footer: '',
		whatsapp_template_back_in_stock_customer_body: '',
	} );
	...
	useSelect( ( select ) =>
		select( 'shop-arraycodes-order-notifications-woocommerce' ).getBackInStock()
	);

	const handleSetStorageAndReloadPage = () => {
		window.localStorage.setItem( 'activeComponent', 'item-13' );
	...
	useEffect( () => {
		if ( getFields ) {
			...
			if ( fetchedFields.whatsapp_template_back_in_stock_customer_id ) {
				setIsHaveId( true );
			}
		}
	}, [ getFields ] );
	...
	const handleSave = async () => {
		try {
			...
			await dispatch( 'shop-arraycodes-order-notifications-woocommerce' ).saveSettings(
				{
					whatsapp_template_back_in_stock_customer_name:
					fields.whatsapp_template_back_in_stock_customer_name,
					whatsapp_template_back_in_stock_customer_language:
					fields.whatsapp_template_back_in_stock_customer_language,
					whatsapp_template_back_in_stock_customer_header:
					fields.whatsapp_template_back_in_stock_customer_header,
					whatsapp_template_back_in_stock_customer_footer:
					fields.whatsapp_template_back_in_stock_customer_footer,
					whatsapp_template_back_in_stock_customer_body:
					fields.whatsapp_template_back_in_stock_customer_body,
				},
				currentState
			);
	...
	const handleSentNewBackInStockCustomer = async () => {
		const dataToSend = {
			fields: getFields,
		};
	...
	const handleSentUpdateBackInStockCustomer = async () => {
		const dataToSend = {
			fields: getFields,
		};
		const arraycodesOnVars = window.arraycodesOnVars;
	...
	const handleSentDeleteBackInStockCustomer = async () => {
		const dataToSend = {
			fields: getFields,
		};
		const arraycodesOnVars = window.arraycodesOnVars;
	...
	return (
		<div className="my-gutenberg-form">
			<SectionHeader
				title={__(
					'Message Template Back In Stock Customer',
			...
			<div className="form-legend">
				<Tip>
					{__(
						'Create the template message for back in stock messages to the customer.',
						'arraycodes-order-notifications-woocommerce'
					)}{' '}
...
export default PageFormTemplateBackInStockCustomer;
