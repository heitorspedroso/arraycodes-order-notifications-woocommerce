...
const PageFormTemplateNewOrder = () => {
	...
	const handleSelectVariableFromParent = ( variable ) => {
		handleChange(
			fields.whatsapp_template_new_order_body + variable,
	...
	useSelect( ( select ) =>
		select( 'shop-arraycodes-order-notifications-woocommerce' ).getOrderSeller()
	);

	const handleSetStorageAndReloadPage = () => {
		window.localStorage.setItem( 'activeComponent', 'item-3' );
	...
	useEffect( () => {
		if ( fieldsFromStore ) {
			...
			if ( fieldsFromStore.whatsapp_template_new_order_id ) {
				setIsHaveId( true );
			} else {
				setIsHaveId( false );
			}
	...
	const handleSave = async () => {
		try {
			...
			await dispatch( 'shop-arraycodes-order-notifications-woocommerce' ).saveSettings(
				{
					whatsapp_template_new_order_name:
						fields.whatsapp_template_new_order_name,
					whatsapp_template_new_order_language:
						fields.whatsapp_template_new_order_language,
					whatsapp_template_new_order_header:
						fields.whatsapp_template_new_order_header,
					whatsapp_template_new_order_footer:
						fields.whatsapp_template_new_order_footer,
					whatsapp_template_new_order_body:
						fields.whatsapp_template_new_order_body,
				},
				fieldsFromStore
			);
	...
	const handleSentNewOrderSeller = async () => {
		const dataToSend = {
			fields,
		};
		...
		try {
			...
			const response = await fetch(
				'/wp-json/arraycodes-order-notifications-woocommerce/v1/new-order-seller',
				fetchOptions
			);
	...
	const handleSentUpdateOrderSeller = async () => {
		const dataToSend = {
			fields
		};
		const arraycodesOnVars = window.arraycodesOnVars;
		...
		try {
			...
			const response = await fetch(
				'/wp-json/arraycodes-order-notifications-woocommerce/v1/update-order-seller',
				fetchOptions
			);
	...
	const handleSentDeleteOrderSeller = async () => {
		const dataToSend = {
			fields
		};
		const arraycodesOnVars = window.arraycodesOnVars;
		...
		try {
			...
			const response = await fetch(
				'/wp-json/arraycodes-order-notifications-woocommerce/v1/delete-order-seller',
				fetchOptions
			);
							...
							<table className="wp-list-table widefat fixed striped table-view-list">
								...
								<tbody>
									<tr>
										<td>
											{
												fields.whatsapp_template_new_order_id
											}
										</td>
										<td>
											{
										...
										<td>
											{ ! isLoadingStatus ? (
												...
												<>
													{
														fields.whatsapp_template_new_order_status
													}
													{ fields.whatsapp_template_new_order_category ===
														'MARKETING' && (
														<div
															style={ {
																marginTop:
...
export default PageFormTemplateNewOrder;
