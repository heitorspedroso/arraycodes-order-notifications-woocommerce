...
const PageFormTemplateOrderDetailsCustomer = () => {
	...
	const handleSelectVariableFromParent = ( variable ) => {
		handleChange(
			fields.whatsapp_template_order_details_customer_body + variable,
			'whatsapp_template_order_details_customer_body'
		);
		closeVariableModal();
	};
	...
	const [ fields, setFields ] = useState( {
		whatsapp_template_order_details_customer_id: '',
		whatsapp_template_order_details_customer_status: '',
		whatsapp_template_order_details_customer_name: '',
		whatsapp_template_order_details_customer_language: '',
		whatsapp_template_order_details_customer_header_type: '',
		whatsapp_template_order_details_customer_header: '',
		whatsapp_template_order_details_customer_footer: '',
		whatsapp_template_order_details_customer_body: '',
	} );
	...
	useSelect( ( select ) =>
		select( 'shop-arraycodes-order-notifications-woocommerce' ).getOrderDetailsCustomer()
	);

	const handleSetStorageAndReloadPage = () => {
		window.localStorage.setItem( 'activeComponent', 'item-8' );
	...
	useEffect( () => {
		if ( fieldsFromStore ) {
			...
			if ( fieldsFromStore.whatsapp_template_order_details_customer_id ) {
				setIsHaveId( true );
			} else {
				setIsHaveId( false );
			}
	...
	const headerType = fields.whatsapp_template_order_details_customer_header_type;
	...
	const handleSave = async () => {
		try {
			...
			await dispatch( 'shop-arraycodes-order-notifications-woocommerce' ).saveSettings(
				{
					whatsapp_template_order_details_customer_name:
						fields.whatsapp_template_order_details_customer_name,
					whatsapp_template_order_details_customer_language:
						fields.whatsapp_template_order_details_customer_language,
					whatsapp_template_order_details_customer_header_type:
						fields.whatsapp_template_order_details_customer_header_type,
					whatsapp_template_order_details_customer_header:
						fields.whatsapp_template_order_details_customer_header,
					whatsapp_template_order_details_customer_footer:
						fields.whatsapp_template_order_details_customer_footer,
					whatsapp_template_order_details_customer_body:
						fields.whatsapp_template_order_details_customer_body,
				},
				fieldsFromStore
			);
	...
	const handleSentNewOrderCustomer = async () => {
		...
		try {
			...
			const response = await fetch(
				'/wp-json/arraycodes-order-notifications-woocommerce/v1/new-order-details-customer',
				fetchOptions
			);
	...
	const handleSentUpdateOrderCustomer = async () => {
		...
		try {
			...
			const response = await fetch(
				'/wp-json/arraycodes-order-notifications-woocommerce/v1/update-order-details-customer',
				fetchOptions
			);
	...
	const handleSentDeleteOrderCustomer = async () => {
		...
		try {
			...
			const response = await fetch(
				'/wp-json/arraycodes-order-notifications-woocommerce/v1/delete-order-details-customer',
				fetchOptions
			);
	...
	return (
		<div className="my-gutenberg-form">
			<SectionHeader
				title={ __(
					'Message Template Order Details Customer',
					'arraycodes-order-notifications-woocommerce'
				) }
			/>
						...
						<>
							<table className="wp-list-table widefat fixed striped table-view-list">
								...
								<tr>
									<td>
										{
											fields.whatsapp_template_order_details_customer_id
										}
									</td>
									<td>
										{
									...
									<td>
										{ ! isLoadingStatus ? (
											...
											<>
												{
													fields.whatsapp_template_order_details_customer_status
												}
											</>
										) }
									</td>
							...
							<div className="form-fields">
								...
								<div className="add-variable">
									...
									<AddVariableModal
										...
										allowedVariables={ [ '{{transactionID}}', '{{transactionTax}}', '{{transactionShipping}}', '{{transactionTotal}}', '{{dateCreated}}', '{{firstName}}', '{{userName}}', '{{userPhone}}', '{{userAddress}}', '{{products}}', '{{shippingName}}', '{{paymentName}}', '{{customField}}' ] }
									/>
								</div>
...
export default PageFormTemplateOrderDetailsCustomer;
