...
const PageFormTemplateAbandonedCartCustomer = () => {
	...
	const handleSelectVariableFromParent = ( variable ) => {
		handleChange(
			fields.whatsapp_template_abandoned_cart_customer_body + variable,
			'whatsapp_template_abandoned_cart_customer_body'
		);
		closeVariableModal();
	};
	...
	const [ fields, setFields ] = useState( {
		whatsapp_template_abandoned_cart_customer_id: '',
		whatsapp_template_abandoned_cart_customer_status: '',
		whatsapp_template_abandoned_cart_customer_name: '',
		whatsapp_template_abandoned_cart_customer_language: '',
		whatsapp_template_abandoned_cart_customer_header: '',
		whatsapp_template_abandoned_cart_customer_footer: '',
		whatsapp_template_abandoned_cart_customer_body: '',
		whatsapp_template_abandoned_cart_customer_minutes_trigger: '',
	} );
	...
	useSelect( ( select ) =>
		select( 'shop-arraycodes-order-notifications-woocommerce' ).getAbandonedCart()
	);

	const handleSetStorageAndReloadPage = () => {
		window.localStorage.setItem( 'activeComponent', 'item-6' );
	...
	useEffect( () => {
		if ( fieldsFromStore ) {
			...
			if (
				fieldsFromStore.whatsapp_template_abandoned_cart_customer_id
			) {
				setIsHaveId( true );
			} else {
				setIsHaveId( false );
	...
	const handleSave = async () => {
		try {
			...
			await dispatch( 'shop-arraycodes-order-notifications-woocommerce' ).saveSettings(
				{
					whatsapp_template_abandoned_cart_customer_name:
						fields.whatsapp_template_abandoned_cart_customer_name,
					whatsapp_template_abandoned_cart_customer_language:
						fields.whatsapp_template_abandoned_cart_customer_language,
					whatsapp_template_abandoned_cart_customer_header:
						fields.whatsapp_template_abandoned_cart_customer_header,
					whatsapp_template_abandoned_cart_customer_footer:
						fields.whatsapp_template_abandoned_cart_customer_footer,
					whatsapp_template_abandoned_cart_customer_body:
						fields.whatsapp_template_abandoned_cart_customer_body,
					whatsapp_template_abandoned_cart_customer_minutes_trigger:
						fields.whatsapp_template_abandoned_cart_customer_minutes_trigger,
				},
				fieldsFromStore
			);
	...
	const handleSentNewAbandonedCartCustomer = async () => {
		const dataToSend = {
			fields,
		};
	...
	const handleSentUpdateAbandonedCartCustomer = async () => {
		const dataToSend = {
			fields,
		};
		const arraycodesOnVars = window.arraycodesOnVars;
	...
	const handleSentDeleteAbandonedCartCustomer = async () => {
		const dataToSend = {
			fields,
		};
		const arraycodesOnVars = window.arraycodesOnVars;
...
export default PageFormTemplateAbandonedCartCustomer;
